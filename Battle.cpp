...
Battle::Battle(Game* ptrGame, Player* player, char grade)
{
	gamePtr = ptrGame;
	diffPtr = gamePtr->getDifficultySelector();
	playerPtr = player;
	goldPtr = gamePtr->getGoldPtr();
	woodPtr = gamePtr->getInventoryPtr()->getWoodPtr();
	stonePtr = gamePtr->getInventoryPtr()->getStonePtr();
	inventoryPtr = gamePtr->getInventoryPtr();
...
void Battle::initWorld()
{
	...
	for (int j = 2; j < 5; j++) {
		if (gradeType == 'N') {
			...
			if (randomNumber == 0) {
				eType[2] = new Alphawolf(diffPtr->getDifficultyGrade());
				eType[2]->setEnemyGrade();
			}
			else if (randomNumber == 1) {
				eType[2] = new Giantgoblin(diffPtr->getDifficultyGrade());
				eType[2]->setEnemyGrade();
			}
			else if (randomNumber == 2) {
				eType[2] = new Orc(diffPtr->getDifficultyGrade());
				eType[2]->setEnemyGrade();
			}

			tileList[2] = new Tile(eType[2]->getType()); // Enemy
			tileList[2]->setTilePosition(gridWidth, gridHeight, 2, 17);
		}
		else if (gradeType == 'B') {
			for (int i = 0; i < 2; i++)
			{
			...
			randomNumber = distr1(generator);

			if (randomNumber == 0) {
				eType[2] = new Demon(diffPtr->getDifficultyGrade());
				eType[2]->setEnemyGrade();	
			}
			else if (randomNumber == 1) {
				eType[2] = new Wolflord(diffPtr->getDifficultyGrade());
				eType[2]->setEnemyGrade();
			}
	...
	while (true) {
		...
		case 122:
			if (currentSelectionRow == 0 && currentSelectionCol == 0) {
				...
				if (inventoryPtr->handleItemSection()) {
					break;
				}
			}
...
bool Battle::enemySelect()
{
	...
	while (true) {
		...
		std::cout << "\n";
		std::cout << "Your Stats: \n";
		std::cout << "HP: " << playerPtr->getFinalHealth() << std::endl;
		std::cout << "ATK: " << playerPtr->getFinalAttack() << std::endl;
		std::cout << "DEF: " << playerPtr->getFinalDefense() << std::endl;
		std::cout << "SPD: " << playerPtr->getFinalSpeed() << std::endl;
