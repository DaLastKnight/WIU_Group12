...
Alphawolf::Alphawolf(char difficultyGrade)
{
	setStats(25, 7, 2, 12);
	setType('A');
...
Alphawolf::~Alphawolf()
{
	std::cout << "Alphawolf has been killed" << std::endl;
}

void Alphawolf::attacking(Entity* target)
{
	target->setStats(target->getHealth() - this->getAttack(), target->getAttack(), target->getDefense(), target->getSpeed());
}

void Alphawolf::setEnemyGrade()
{
	grade = 'E';
}

char Alphawolf::getEnemyGrade()
{
	return grade;
}
